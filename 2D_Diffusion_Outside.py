...
domain, cell_markers, facet_markers = gmshio.read_from_msh("outside.msh", MPI.COMM_WORLD, gdim=2)
...
for i in range(x.shape[0]):
    ...
    if np.isclose(midpoint[0]**2+midpoint[1]**2,o*o):
        init.vector.setValueLocal(i, O_value)
    else:
...
boundaries =[(1, lambda x: np.isclose(np.sqrt(x[0]**2+x[1]**2),R)),
            (2, lambda x: np.isclose(np.sqrt(x[0]**2+x[1]**2),o))]
...
with XDMFFile(domain.comm, "facet_tags_outside.xdmf", "w") as xdmf:
...
xdmf = XDMFFile(domain.comm, "2D_Diffusion_Outside.xdmf", "w")
